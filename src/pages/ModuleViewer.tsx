...
export default function ModuleViewer() {
  ...
  const track = mockTracks.find(tr => tr.id === trackId);
  ...
  const handlePlay = () => {
    setIsPlaying(true);
    // Simulate video completion after a short time for demo purposes
    setTimeout(() => {
      setVideoCompleted(true);
      setIsPlaying(false);
    }, 5000);
      ...
      <div className="glass-panel-elevated overflow-hidden">
        <div className="relative aspect-video bg-muted/50 flex items-center justify-center">
          ...
          {isPlaying && !videoCompleted && (
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-foreground font-medium animate-pulse">
                {t('Video is playing...', 'वीडियो चल रहा है...')}
              </p>
              <p className="text-xs text-muted-foreground">
                {t('(Demo: completes in 5 seconds)', '(डेमो: 5 सेकंड में पूरा होगा)')}
              </p>
            </div>
          )}
          ...
          {videoCompleted && (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                {t('Choose what to do next', 'आगे क्या करना है चुनें')}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="h-1 bg-muted">
          <div 
            ...
            style={{ width: videoCompleted ? '100%' : isPlaying ? '100%' : '0%' }}
          />
        </div>
      </div>

      {/* Post-video Actions - shown after video completes */}
      {videoCompleted && (
        <div className="glass-panel-elevated p-6 animate-fade-in space-y-4">
          <h3 className="font-semibold text-foreground">
            {t('What would you like to do?', 'आप क्या करना चाहेंगे?')}
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <Button
              variant="outline"
              className="flex flex-col h-auto py-4 gap-2"
              onClick={() => { setVideoCompleted(false); setIsPlaying(false); }}
            >
          ...
          <div className="flex justify-between pt-4 border-t border-border">
            ...
            <Button
              disabled={!nextModule}
              onClick={() => nextModule && goToModule(nextModule.id)}
            >
              {t('Next', 'अगला')}
